...
// En coach.js - Endpoint de medidas físicas
router.post('/client-measurements', authMiddleware, async (req, res) => {
  try {
    console.log('==== Registrando medidas físicas ====');
    console.log('Datos recibidos:', JSON.stringify(req.body, null, 2));
    
    const { id_usuario, ...measurementData } = req.body;
    
    // ... validaciones y procesamiento de datos como antes ...
    
    // Insertar las medidas físicas
    const result = await pool.request()
      .input('id_usuario', sql.Int, clientId)
      .input('peso', sql.Decimal(5, 2), processedData.peso)
      // ... resto de inputs ...
      .query(`
        INSERT INTO Medidas_Corporales (
          id_usuario,
    ...
    // Crear notificación para el cliente usando un tipo válido existente
    await pool.request()
      .input('id_usuario', sql.Int, clientId)
      .input('id_origen', sql.Int, req.user.id)
      // Usar 'verificacion_datos' que es uno de los tipos permitidos
      .query(`
        INSERT INTO Notificaciones (
          id_usuario,
          tipo,
...
// En coach.js - al principio del archivo o en un script de inicialización
const updateNotificacionesTipoConstraint = async () => {
  try {
    const pool = await connectDB();
    
    console.log('Actualizando la restricción CHECK para la columna tipo en Notificaciones');
    
    // Eliminar la restricción existente
    await pool.request().query(`
      IF EXISTS (SELECT * FROM sys.check_constraints WHERE name = 'CK__Notificaci__tipo__07C12930')
      BEGIN
          ALTER TABLE Notificaciones
          DROP CONSTRAINT CK__Notificaci__tipo__07C12930
      END
    `);
    
    // Crear una nueva restricción con todos los tipos, incluido nuevas_medidas
    await pool.request().query(`
      ALTER TABLE Notificaciones
      ADD CONSTRAINT CK_Notificaciones_Tipo
      CHECK (tipo IN ('asignacion_coach', 'nueva_rutina', 'solicitud_entrenamiento', 'verificacion_datos', 'nuevas_medidas'))
    `);
    
    console.log('Restricción CHECK actualizada correctamente');
  } catch (error) {
    console.error('Error al actualizar la restricción CHECK:', error);
  }
};

// Ejecutar esta función al iniciar el servidor
updateNotificacionesTipoConstraint();
...
router.post('/assign-routine', authMiddleware, verifyCoach, async (req, res) => {
  try {
    ...
    try {
      // Verificar y eliminar asignaciones existentes para los mismos días
      for (const dia of dias_entrenamiento) {
        // Identificar asignaciones que tienen este día
        const existingAssignmentsResult = await new sql.Request(transaction)
          .input('clientId', sql.Int, clientId)
          .input('dia', sql.NVarChar, dia)
          .query(`
        ...
        // Para cada asignación existente, eliminar primero el día de entrenamiento
        for (const assignment of existingAssignmentsResult.recordset) {
          await new sql.Request(transaction)
            .input('id_asignacion_rutina', sql.Int, assignment.id_asignacion_rutina)
            .input('dia', sql.NVarChar, dia)
            .query(`
              DELETE FROM Dias_Entrenamiento
              WHERE id_asignacion_rutina = @id_asignacion_rutina 
                AND dia_semana = @dia
            `);
            
          // Verificar si la asignación ya no tiene días asociados
          const remainingDaysResult = await new sql.Request(transaction)
            .input('id_asignacion_rutina', sql.Int, assignment.id_asignacion_rutina)
            .query(`
          ...
          // Si ya no hay días asociados, marcar la asignación como completada
          if (remainingDaysResult.recordset[0].remaining_days === 0) {
            await new sql.Request(transaction)
              .input('id_asignacion_rutina', sql.Int, assignment.id_asignacion_rutina)
              .query(`
                UPDATE Asignaciones_Rutina
      ...
      // Verificar si ya existe una asignación activa para esta rutina y cliente
      const existingAssignmentResult = await new sql.Request(transaction)
        .input('routineId', sql.Int, routineId)
        .input('clientId', sql.Int, clientId)
        .query(`
          ...
          FROM Asignaciones_Rutina
          WHERE id_rutina = @routineId
            AND id_usuario = @clientId
            AND estado = 'activa'
        `);
        
      let id_asignacion_rutina;
      
      if (existingAssignmentResult.recordset.length > 0) {
        // Si ya existe una asignación para esta rutina, usarla
        id_asignacion_rutina = existingAssignmentResult.recordset[0].id_asignacion_rutina;
      } else {
        // Crear una nueva asignación
        const insertAssignmentResult = await new sql.Request(transaction)
          .input('routineId', sql.Int, routineId)
          .input('clientId', sql.Int, clientId)
          .input('fecha_asignacion', sql.DateTime, new Date())
          .query(`
        ...
        id_asignacion_rutina = insertAssignmentResult.recordset[0].id_asignacion_rutina;
      }
      
      // Insertar los días de entrenamiento para esta asignación
      for (const dia of dias_entrenamiento) {
        // Verificar si este día ya está asignado para esta asignación
        const existingDayResult = await new sql.Request(transaction)
          .input('id_asignacion_rutina', sql.Int, id_asignacion_rutina)
          .input('dia_semana', sql.NVarChar, dia)
          .query(`
            SELECT id_dia_entrenamiento
            FROM Dias_Entrenamiento
            WHERE id_asignacion_rutina = @id_asignacion_rutina
              AND dia_semana = @dia_semana
          `);
          
        // Si este día no está asignado para esta asignación, insertarlo
        if (existingDayResult.recordset.length === 0) {
          await new sql.Request(transaction)
            .input('id_asignacion_rutina', sql.Int, id_asignacion_rutina)
            .input('dia_semana', sql.NVarChar, dia)
            .query(`
...
// Añadir a coach.js
router.get('/assignment/:assignmentId/days', authMiddleware, async (req, res) => {
  try {
    const { assignmentId } = req.params;
    
    console.log('==== Obteniendo días de entrenamiento ====');
    ...
    // Verificar que la asignación pertenece a un cliente de este coach
    const assignmentCheckResult = await pool.request()
      .input('id_asignacion_rutina', sql.Int, assignmentId)
      .input('id_coach', sql.Int, coachId)
      .query(`
        SELECT ar.id_asignacion_rutina
        FROM Asignaciones_Rutina ar
        JOIN Asignaciones_Coach_Cliente acc ON ar.id_usuario = acc.id_usuario
        WHERE ar.id_asignacion_rutina = @id_asignacion_rutina 
        AND acc.id_coach = @id_coach
        AND acc.estado = 'activa'
      `);
    
    if (assignmentCheckResult.recordset.length === 0) {
      return res.status(404).json({ message: 'Asignación no encontrada o no tienes acceso a ella' });
    }
    ...
    const daysResult = await pool.request()
      ...
      .query(`
        ...
        FROM 
          Dias_Entrenamiento
        WHERE 
          id_asignacion_rutina = @id_asignacion_rutina
        ORDER BY
          CASE dia_semana
            WHEN 'lunes' THEN 1
            WHEN 'martes' THEN 2
    ...
    res.json(daysResult.recordset);
  } catch (error) {
    console.error('Error al obtener días de entrenamiento:', error);
    res.status(500).json({ message: 'Error al obtener días de entrenamiento' });
  }
});
// Obtener detalles de una rutina específica
